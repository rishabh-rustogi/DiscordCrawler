...
def requestURLResponse(url, token, params):
    ...
    while response.status_code == 429:
        print("Global Rate Limit for invalid requests exceeded")
        print('[Pausing for {}s] | Requested URL: {}'.format(
    ...
    if response.status_code != 200:
        print('Error while requesting URL: {}'.format(response.json()))
        exit(1)
...
def downloadFolder(bucketName, prefix, destination):
    print('Downloading folder from GCP Storage (Bucket: {}, Prefix: {}, Destination: {})'.format(
        bucketName, 
        prefix, 
        destination))
...
def uploadFolder(bucketName, prefix, source):
    print('Uploading folder to GCP Storage (Bucket: {}, Prefix: {}, Source: {})'.format(
        bucketName, 
        prefix, 
        source))
...
def uploadFile(bucketName, sourceFile, destinationFile):
    print('Uploading file to GCP Storage (Bucket: {}, Source: {}, Destination: {})'.format(
        bucketName, 
        sourceFile, 
        destinationFile))
...
#     print('Publishing message to Pub/Sub (Topic: {})'.format(topicName))
#     publisher = pubsub_v1.PublisherClient()
#     topic_path = publisher.topic_path('discord-message-extractor', topicName)
#     future = publisher.publish(topic_path, message.encode('utf-8'))
#     print('({})Message published: {}'.format(future.result(), message))
...
def writeFile(path, json_data):
    ...
    try:
        ...
        print('Error while writing file: {}'.format(e))  
        exit(1)
...
def updateConfigs():
    ...
    for user in user_token:
        ...
        for guild in guilds:
            ...
            if guild['id'] not in ['735708716128796763']:#, '894349669499568148']:
                continue
...
def downloadContent(url, path):
    try:
        ...
        if fileSize <= download_attachments_MAX_SIZE:
            with open(path + fileName, 'wb') as f:
                f.write(requests.get(url).content)
        else:
            print('File too large ({}): {}'.format(fileSize, url))
    except Exception as e:
        print('Error while downloading file: {}'.format(e))  
        exit(1)

'''
Check if message content passes all the regex filters
@param message: Message to check
'''
def checkFiltersOnMessage(message):
    content = message['content']
    if content is None or len(content) == 0:
        return False

    if len(regex_filter_expression) == 0:
        return True
...
def extractMessageFromExploredChannels():
            ...
            for channel in user_server_channel[user][guild]:
                ...
                try:
                    ...
                    if user_server_channel[user][guild][channel]['status'] == "processing":
                        print('Extracting messages from channel (User: {}, Guild: {}, Channel: {})'.format(
                            user, 
                            guild, 
                            channel))
                        ...
                        if "attachments" in messages[0] and download_attachments:
                            for attachment in messages[0]["attachments"]:
                                if 'url' in attachment:
                                    downloadContent(attachment['url'], DATA_FOLDER_MEDIA)

                        # Update the JSON object with the messages if the message passes the regex filters
                        if checkFiltersOnMessage(messages[0]):
                            messages_JSON['messages'] = messages
                        else:
                            messages_JSON['messages'] = []

                        # Set the BEFORE param to the timestamp of the latest message processed
                        last_message_processed = messages[0]['id']
                        ...
                        latest_message_processed = messages[-1]['id']
                        ...
                        while (check):
                            ...
                            for message in messages:
                                ...
                                if "attachments" in message and download_attachments:
                                    for attachment in message["attachments"]:
                                        if 'url' in attachment:
                                            downloadContent(attachment['url'], DATA_FOLDER_MEDIA)

                                # Write the messages to the holder if the message passes the regex filters
                                if checkFiltersOnMessage(message):
                            ...
                            messages_JSON['messages'] += temporary_messages_holder
                        ...
                        # Write the JSON object to a file if the messages list is not empty
    ...
    print('Uploading updated  configs')
    # uploadFolder(BUCKET_NAME, 'configs/', 'configs/')

    # Upload the Data folder to GCP Storage
    print('Uploading extracted messages')
...
if __name__ == "__main__":
    ...
    if mode == 'update':
        ...
        print('Running in extract mode: OLD')
        extractMessageFromExploredChannels()
    elif mode == 'extractNew':
        print('Running in extract mode: NEW')
        extractMessageFromNewChannels()
    elif mode == 'extractAll':
        print("Extracting all")
        print('Running in do all mode')
        updateConfigs()
        extractMessageFromExploredChannels()
        extractMessageFromNewChannels()
    else:
