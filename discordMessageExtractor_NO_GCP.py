...
Currently Keeping them here, until finalizing
"""
...
def request_url_response(url, token, params):
...
# def download_folder(bucket_name, prefix, destination):
...
#     * destination: str -- Destination of the downloaded folder
#     """
...
#     for blob in blobs:
#         blob.download_to_file_name(destination + blob.name.split('/')[-1])


# def upload_folder(bucket_name, prefix, source):
...
#     * params: str -- Source of the folder to upload
#     """
...
#     upload_folder_recursively(bucket_name, prefix, source)


# def upload_folder_recursively(bucket_name, prefix, source):
#     """ Uploads a File to GCP Storage with level deeper than 1

#     Keyword Arguments:
#     * bucket_name: str -- Name of the bucket to upload the file to
#     * prefix: str -- Prefix of the folder to upload
#     * source: str -- Source of the file to upload
#     """

#     relative_paths = glob.glob(source + '**', recursive=True)
#     for relative_path in relative_paths:
#         if os.path.isfile(relative_path):
#             upload_file(bucket_name, relative_path, prefix + relative_path.replace(source, ''))
...
#     # Upload the file
#     blob.upload_from_file_name(source_file)
...
# def publish_to_pub_sub(topic_name, message):
#     """ Publishes messages to a Pub/Sub topic

#     Keyword Arguments:
#     * topic_name: str -- Name of the Pub/Sub topic to publish to
#     * message: str -- Messages to publish to the topic
#     """

#     logging.info("Publishing message to Pub/Sub (Topic: {})".format(topic_name))
#     publisher = pubsub_v1.PublisherClient()
#     topic_path = publisher.topic_path('discord-message-extractor', topic_name)
#     future = publisher.publish(topic_path, message.encode('utf-8'))
#     logging.info("({})Message published: {}".format(future.result(), message))


def read_config_as_json(path = 'configs/user_token.json'):
...
def write_file(path, json_data):
    ...
    * json_data: json object -- JSON data to dump to the file
    ...
    create_folder('/'.join(file_route[ : -1]))
    try:
...
def update_configs():
    ...
    user_token = read_config_as_json()
    user_server_channel = read_config_as_json('configs/user_server_channel_DO_NOT_EDIT.json')
    ...
    for user in user_token:
        ...
        guilds = request_url_response(BASE_URL + urls['guilds'], 
        ...
        for guild in guilds:
            ...
            channels = request_url_response(BASE_URL + urls['channels'].format(guild['id']), 
                                            user_token[user]['token'], 
                                            url_params['channels'])
    ...
    write_file('configs/user_server_channel_DO_NOT_EDIT.json', user_server_channel)
...
def create_base_message_json(user_id, guild_id, channel_id, channel_name):
...
def twitter_snowflake_to_datetime(snowflake):
...
def extract_message_from_explored_channels():
        ...
        for guild in user_server_channel[user]:
            ...
            for channel in user_server_channel[user][guild]:
                ...
                if user_server_channel[user][guild][channel]['status'] == 'processing':
                    ...
                    # Get the last message timestamp from the config file
                    last_proccessed_timestamp = twitter_snowflake_to_datetime(user_server_channel[user][guild][channel]['last_processed'])
                    
                    # Create a new JSON object for the channel
                    messages_JSON = create_base_message_json(user, guild, channel, user_server_channel[user][guild][channel]['name'])
                    
                    # Request the latest messages from the channel
                    messages = request_url_response(BASE_URL + urls['messages'].format(channel), 
                                                    user_token[user]['token'], 
                                                    {'limit': 1})
                    
                    # If the latest message timestamp is <= than the last message processed, ignore the channel
                    current_timestamp = twitter_snowflake_to_datetime(messages[0]['id'])
                    ...
                    if "attachments" in messages[0]:
                        for attachment in messages[0]['attachments']:
                            if 'url' in attachment:
                                download_content(attachment['url'], DATA_FOLDER_MEDIA)
                    ...
                    # Write the JSON object to a file
                    write_file(DATA_FOLDER + '{}.json'.format(channel), messages_JSON)
    ...
    # upload_folder(BUCKET_NAME, DATA_FOLDER, DATA_FOLDER)


def extract_message_from_new_channels():
...
if __name__ == "__main__":
    ...
    if args.mode == 'update':
        ...
        extract_message_from_explored_channels()
    elif args.mode == 'extractNew':
        logging.info("Running in extract mode: NEW")
        extract_message_from_new_channels()
